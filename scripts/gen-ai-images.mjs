...
// Crash-safe: write both the cache and the resolved registry after every entry,
// so an interrupted run always leaves media-ai.ts consistent with the swapped covers.
const writeRegistry = async () => {
  await writeFile(cacheFile, JSON.stringify(out, null, 2));
  await writeFile(
    join(root, 'src', 'lib', 'media-ai.ts'),
